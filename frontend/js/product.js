...
const TYPE_CONFIG = {
  ...
  ram: {
    ...
    specs: {
      'Capacity':  'ramsize',
      'Type':      'ramtype',
      'Kit':       d => d.quantity ? `${d.quantity}× stick(s)` : null,
      'Speed':     'ramspeed',
    }
  },
  storage: {
...
function displayProduct(data, config) {
  ...
  document.title = `${data[config.nameField] || 'Product'} — PC Configurator`;

  document.getElementById('product-img').src =
    data.imgpath && data.imgpath !== '' ? data.imgpath : fallbackImg;

  document.getElementById('product-img').alt = data[config.nameField] || 'Product';
  ...
  document.getElementById('product-name-breadcrumb').textContent = data[config.nameField] || '—';
  document.getElementById('product-name').textContent = data[config.nameField] || 'Unknown Product';

  const price = data.price != null ? `$${data.price.toLocaleString()}` : 'N/A';
